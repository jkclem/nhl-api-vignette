...
Now let's look at shooting percentage vs. shots per game. I added a color gradient for the win-loss-ratio. 
...
To do this, I'm calculating the the penalty minutes per game as $\frac{\text{Total Penalty Minutes}}{\text{Total Games Played}}$. I again calculate the win-loss ratio, because I want to see how penalty minutes relate to a team's record. I'm also interested in how a game being a playoff or regular season game affects penalty minutes. I think the importance of a playoff game might cause changes in the penalty behavior.
...
knitr::kable(table(activeTeams$gameType, activeTeams$recordType),
             caption="Counts of Franchise Record Types by Game Type for Active Franchises")
```
...
inactiveTeams[currentEndRow+seq(numDidntMakePlayoffs), "recordType"] <- "Losing Record"
inactiveTeams[currentEndRow+seq(numDidntMakePlayoffs), "gameType"] <- "Playoffs"

knitr::kable(table(inactiveTeams$gameType, inactiveTeams$recordType),
             caption="Counts of Franchise Record Types by Game Type for Inactive Franchises")
```

```{r}
winPercSumm <- teamTotalStats %>% 
  select(gameType, winPercentage) %>%
  filter(is.na(winPercentage) == FALSE) %>%
  group_by(gameType) %>%
...
plot4 <- plot4 + scale_y_continuous("Win Percentage")
plot4 <- plot4 + ggtitle("Franchise Win Percentage by Game Type")
...
The histogram below provides another way to visualize the distribution of a variable. It's clear the penalty minutes per playoff game has a right tail stretching beyond regular season games.

```{r}
plot5 <- ggplot(teamTotalStats,aes(penaltyMinutesPerGame, y=..density.., fill=gameType)) + 
