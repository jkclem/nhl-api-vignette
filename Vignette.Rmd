...
I wrote this function to interact with the `franchise` endpoint of the NHL Stats API. It returns a `data.frame` containing the current season stats. It takes one argument; `team`, which can either be `"all"`, the full name of a team (e.g. `"New Jersey Devils"`), or the most recent team Id (e.g. `1` for the New Jersey Devils).
...
There doesn't appear to be a clear relationship between those two variables, which I thought there might be.
...
To get numbers on the same basis when the number of games played are so different, I'm calculating the the penalty minutes per game as $\frac{\text{Total Penalty Minutes}}{\text{Total Games Played}}$. I again calculate the win percentage as $\text{Win Percentage} = \frac{\text{Wins}}{\text{Games Played}}$. I'm also interested in how a game being a playoff or regular season game affects penalty minutes because I think the importance of a playoff game might cause changes in the penalty behavior.
...
Now let's get a numerical summary of the win percentage by the game type.
...
Given the t-statistics for the regression coefficients, a quadratic relationship appears reasonable. This model explains `r print(paste0(winVarExplainedPerc, "%"))` of the variation in win percentage. This is an interesting find!
