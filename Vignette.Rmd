...
Additionally, I used the following packages for this analysis:
...
currentSeason <- currentSeason %>%
  mutate(win.loss.ratio = as.numeric(stat.wins) / as.numeric(stat.losses))

# Create a ggplot object to plot shooting pct vs. shots per game.
plot <- ggplot(currentSeason, 
               aes(as.numeric(stat.shotsPerGame),
                   as.numeric(stat.shootingPctg),
                   color=win.loss.ratio))
# Make a scatter plot.
plot <- plot + geom_point(size=4, alpha=0.75)
# Adjust the color gradient to go from blue at the low end, to red at the high
# end of win.loss.ratio
plot <- plot + scale_color_gradient(low="blue", high="red",
                                    name="Wins / Losses")
# Add an x-axis label.
plot <- plot + scale_x_continuous("Shots per Game")
# Add an y-axis label.
plot <- plot + scale_y_continuous("Shooting Percentage")
# Add a title.
plot <- plot + ggtitle("Shooting Percentage vs. Shots per Game")
